...
class DFS_DLSLogic(BaseSearchLogic):
    ...
    def dls(self, start_node, goal_node=None, depth_limit=None):
        self.update_node_color(goal_node, COLOR_GOAL)
        # Calculate depth limit or set to infinity if no goal node
        depth_limit = 5  # self.calculate_goal_depth(start_node, goal_node) if goal_node else float('inf')
        stack = [(start_node, 0)]
        visited = set()
        current_depth = 0
        ...
        while stack or next_level_stack:
            ...
            if current_node not in visited:
                ...
                if current_node == goal_node:
                    path, _x = self.reconstruct_path(
                        parents, start_node, goal_node, self.node_costs
                    )
                ...
                for neighbor in reversed(self.get_neighbors(current_node)):
                    if neighbor not in visited and all(
                        neighbor != n for n, d in stack + next_level_stack
                    ):
